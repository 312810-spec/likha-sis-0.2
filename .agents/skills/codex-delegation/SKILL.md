...
description: Use when considering delegating an implementation or review task to the official OpenAI Codex plugin (`codex@openai-codex`), or when reviewing work that came back from Codex.
---
...
checkout and machine** a Codex session already has. It is not more
isolated than Codex's own tool access, and it is not less either — it
...
secret/PII-pattern defense-in-depth) are wired to Codex's own
...
## Codex owns architecture; Codex is a bounded worker

Codex decides and never delegates: RBAC/authorization semantics,
...
| Risk       | Examples                                                                                                                                                                                                     | Flow                                                                                                                                                                                                                        |
| ---------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **LOW**    | formatting, repetitive test additions, mechanical refactors, straightforward type fixes, docs explicitly specified by Codex, implementation from an already-proven in-repo pattern                           | Codex instruction → Codex implementation → automated verification (`npm run quality` / `cargo test` etc.) → Codex lightweight diff read                                                                                     |
| **MEDIUM** | repository/application-service changes, UI workflows, migrations following an established pattern (see `local-database` skill's versioned-reference-data shape), native adapters, non-sensitive domain logic | Codex spec → **written implementation contract** (below) → Codex implementation → verification → Codex independent review                                                                                                   |
| **HIGH**   | RBAC, auth, encryption, key storage, sync, tenant/school isolation, learner PII, destructive migrations, backup/recovery, official-form correctness, security-sensitive native code                          | Codex architecture (+ specialist agent challenge, e.g. `security-reviewer`) → written contract → Codex bounded implementation → verification → Codex independent review → security/domain specialist review → accept/reject |

When uncertain which tier applies, use the higher one. Codex may always
override a task's classification upward.

## Implementation contract (Codex → Codex)
...
initiative — the same rules that already bind Codex in this harness.
...
there is no isolation between a concurrent Codex edit and a
Codex-in-progress one).

## Token/context discipline

The plugin adds a real, measured always-on cost (~449 tokens per
session just from being enabled, confirmed via `Codex plugin details
...
needs — don't have it re-derive architecture Codex already worked out,
...
the same `AGENTS.md` → skills/ADRs → Codex orchestration chain every
