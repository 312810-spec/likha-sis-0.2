...
Plan / generate / heal lean on the same mechanic: run `npx playwright@1.49.0 test --debug=cli` in the background, then `playwright-cli attach tw-XXXX` to drive the paused page interactively. See [playwright-tests.md](playwright-tests.md) for the debug/attach mechanics.
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright@1.49.0 test tests/seed.spec.ts --debug=cli
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright@1.49.0 test <seed-file> --debug=cli   # background
playwright-cli attach tw-XXXX
# resume
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright@1.49.0 test tests/<group>/<scenario>.spec.ts
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright@1.49.0 test
```
...
```bash
PLAYWRIGHT_HTML_OPEN=never npx playwright@1.49.0 test tests/<group>/<scenario>.spec.ts:<line> --debug=cli
