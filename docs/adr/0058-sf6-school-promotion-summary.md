...
### Reconciliation security/correctness addendum (2026-09-01)

School-wide SF6 export is gated by the semantic
`Capability::ExportSchoolReports` capability (Registrar + School Head), not
merely by an active Teacher session and not by reusing an unrelated capability.
Recommended: the dedicated capability, because later adding the planned LIS
Coordinator widens one role mapping. Next best: reuse `ManageLearners`; rejected
because learner-record editing and school-wide official-report disclosure are
different authority classes.

Pending/incomplete learners are explicit SF6 columns and are included in
section, grade-level, and school totals. They must never disappear silently
from the total because their grades are incomplete.
