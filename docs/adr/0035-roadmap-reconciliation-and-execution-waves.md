...
| 1    | Foundational primitives                                          | School branding (logo→derived theme), RBAC (role column + enforcement gate, one representative gated feature), curriculum-versioning schema (schema only, no new UI yet). Deliberately small — proves three new primitives cheaply before large feature investment.                                                                                                                                                                                                                                                                                                             |
| 2    | Learner Core (combined UX-05 + SF1)                              | Extend existing search/section-editing/roster-export; add bulk import with conservative, provenance-tracked duplicate reconciliation; learner photo; enrollment history. Gated by Wave 1's RBAC (e.g. Registrar-only bulk import).                                                                                                                                                                                                                                                                                                                                              |
| 3    | Authoritative-template Form Engine                               | The existing CSV-based export engine (`csv.rs`/`FieldDisclosure`, proven 3x: SF2, SF9, learner roster) already covers the "reusable disclosed export" half. What's missing is one representative _authoritative-template_ form, not all ten. Recommended: SF9 (report card) — it extends UX-04's already-working grade computation directly. Build the Tauri → scoped sidecar → Apache POI/HSSF → authoritative `.xls` template pattern once, generalized enough that SF10's later bulk import reuses Wave 2's import/reconciliation architecture rather than a bespoke parser. |
| 4    | Teacher Load + Class Schedule foundation                         | Schema + a representative single-section scheduling proof (not a full constraint solver yet) → feeds SF7.                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 5    | Sync + cloud authorization + session hardening                   | Cloud-target 10-scenario decision **made**: `docs/adr/0042-cloud-sync-target-decision.md` (Cloudflare Workers + Durable Object per school, D1 as documented fallback). One real end-to-end sync round trip remains **blocked** on user-supplied Cloudflare credentials (external material, approval gate #2) — not yet done. Formalize the offline-session/re-authentication product requirement (`PRODUCT-CONTRACT.md` §13) with an actual security-reviewer pass, not a default number.                                                                                       |
| 6    | Teacher Creation Studio integration + Android critical workflows | Research and classify ILAWCraft (ADOPT/PILOT/REFERENCE/REJECT) before any adapter code. Build one representative generator adapter, not the full studio. Android: My Day + Attendance critical-workflow architecture proof.                                                                                                                                                                                                                                                                                                                                                     |
| 7    | Cross-app finish, accessibility, performance, regression gate    | Unchanged in substance from the original UX-08 scope. Final hardening and handoff-readiness pass before/at the high-capability window's end.                                                                                                                                                                                                                                                                                                                                                                                                                                    |
