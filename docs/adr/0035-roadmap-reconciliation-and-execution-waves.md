...
| 5    | Sync + cloud authorization + session hardening                   | Run the actual 10-scenario cloud-target decision (Decision 5 above) before writing sync code. One real end-to-end sync round trip, not a full feature set. Formalize the offline-session/re-authentication product requirement (`PRODUCT-CONTRACT.md` §13) with an actual security-reviewer pass, not a default number.                                                                                                                                                                                                                                                         |
| 6    | Teacher Creation Studio integration + Android critical workflows | ILAWCraft research/classification **complete** (`docs/adr/0042-ilawcraft-research-and-classification.md`) — split ADOPT (COT rubric data)/REFERENCE (generation architecture)/REJECT (payment layer)/overall PILOT. Internally ordered: **6a** item analysis + low-risk classroom utilities (no paid API, no new architecture) before **6b** Teacher Creation Studio (one representative lesson-plan→slide-deck adapter, gated on a human BYOK-vs-LIKHA-funded AI-cost decision before adapter code). Android: My Day + Attendance critical-workflow architecture proof.        |
