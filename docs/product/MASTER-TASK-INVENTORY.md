...
- [x] **School-Laptop Hub Daemon/Service Resilience (Batch 14, 2026-09-09):** In-process listener now retries forever on bind/serve failure with a tested exponential-backoff supervisor (`hub_server::supervisor`, ADR-0085) — codable core done, tested by `cargo test`. Whole-process/reboot recovery is scripted (`ops/hub-daemon-recovery-setup.ps1`, a Windows Scheduled Task registrar) but not yet executed on real hardware — see `docs/VERIFICATION-DEBT.md`'s Batch 14 entry and `ops/hub-daemon-recovery-runbook.md`.
- [x] **Hub Hardware Gates (Batch 14, 2026-09-09):** `ops/hub-hardware-gate-audit.ps1` (BitLocker/firewall/patch pass-fail audit) written with decision logic separated from live data-fetching and unit-tested via `ops/hub-hardware-gate-audit.Tests.ps1` (Pester) — reviewed but not yet executed on a real Windows machine (no `pwsh` in this sandbox). See ADR-0086 and `ops/hub-hardware-gate-audit-runbook.md`.
- [x] **Disaster Recovery Drill (Batch 14, 2026-09-09):** Two-copy encrypted backup mechanism built (`backup::create_two_copy_backup`, `commands::backup::create_disaster_recovery_backup`) using SQLCipher's own `sqlcipher_export()` — never a raw file copy or plaintext dump. Full create→verify→restore round trip proven by 11 real automated tests (`cargo test --lib backup::`/`commands::backup::`), including the encryption-at-rest guarantee. A real human loss-and-restore drill on the physical hub laptop is still owed — see ADR-0087 and `ops/DR-DRILL-RUNBOOK.md`.
...
- [x] **Interim Review Workflow (School-Head-as-approver):** Teacher (or School Head) submits a class record's grades → automated checks (missing summative scores, out-of-bounds values, weight-group mismatch) run and post as notes → School Head approves/rejects with feedback (`repository::grade_submission`, migration 45, `docs/adr/0073-interim-grade-review-pipeline.md`). Superseded (see below) — kept as historical record.
- [x] **Real `master_teacher` RBAC role (Batch 17):** `repository::role::MASTER_TEACHER`, migration 59 (widens `user_school_roles`'s CHECK constraint). Resolves `docs/product/OWNER-DECISIONS-NEEDED.md` item 1 for real — permanent design, not interim. Holding this role grants no School-Head-level capability by itself (`docs/adr/0089-master-teacher-rbac-and-two-tier-grade-review.md`).
- [x] **Teacher Oversight Assignment (Batch 17):** `teacher_oversight_assignments` (migration 60) — a time-scoped, school-scoped assignment table mirroring `section_advisories` exactly (at most one active overseer per teacher, School-Head-only assign/end via `Capability::ManageTeacherOversightAssignments`). `repository::teacher_oversight_assignment` + `commands::teacher_oversight_assignment`.
- [ ] **Two-tier grade-submission decision rewire:** `decide_grade_submission` still uses ADR-0073's single-step School-Head-only decision. Rewiring it to route through the teacher's assigned Master Teacher first, then a distinct School-Head "final lock" step (with the no-MT-assigned fallback to direct School-Head approval), is the recorded next slice — see `docs/CURRENT-HANDOFF.md`'s Batch 17 entry. Not yet started.
- [ ] **Master Teacher / oversight-assignment UI:** two-tier status display on the grade-review screen, School-Head-only oversight-assignment management screen. Deferred with the rewire above (UI depends on the rewired decision shape).
- [x] **Principal Overview Dashboard (interim):** School-wide composite-grade view (`get_principal_overview_dashboard`, reusing `grading_computation`) + submission-status matrix (`list_grade_submissions_for_school`). "Principal" maps to the existing School Head role, same interim substitution as ADR-0073 — the dashboard's principal-role substitution is a separate, not-yet-revisited question from the grade-approval RBAC question this batch resolved.
...
- Deferred: command-level integration tests for the two-tier flow (see `docs/VERIFICATION-DEBT.md`).
...
- [x] **Sub-scope 2.2: Certificate & Recognition Template:** Printable DepEd Academic Excellence Award certificate generator with automated DO 015 honors qualification engine. **Batch 5 (2026-09-08):** eligibility engine + certificate content builder shipped and tested (`src/domain/award-eligibility.ts`, `src/domain/certificate.ts`) with the GA threshold explicitly flagged as unverified/configurable and the disciplinary-anecdotes check explicitly not implemented (no Anecdotal Records feature exists at the time). **Batch 12 (2026-09-08/09, ADR-0083)** built the `AnecdotalRecord` entity (see the Sync Wiring section below) but deliberately did NOT wire it into this check -- that remains a future batch's job. **Batch 8 item 1 (2026-09-08):** printable `CertificateAwardScreen` shipped and wired — loops a section roster through `computeTermGrade` per subject/grading-period, both disclosures render visibly on the eligibility list and on the printed certificate itself, eligible-only "Print certificate" gating, tested (`CertificateAwardScreen.test.tsx`, axe-clean).
...
- [x] `AnecdotalRecord` + `AnecdotalRecordFollowup` (Guidance Records) -- **Batch 12 (2026-09-08/09, ADR-0083)**: built and wired end to end across all 4 checkpoints. Migration 57 (`anecdotal_records` + `anecdotal_record_followups`, append-only follow-up log, `category` a generic positive/negative/neutral classification, deliberately NOT overfit to the disciplinary-only framing this entity was originally motivated by) + migration 58 (entity_kind CHECK widening, bundled into checkpoint 1 for schema-file locality, same pattern Batch 11 used). `repository::anecdotal_record`: tenant-scoped CRUD, `add_followup` INSERT-only. Authorization reuses `auth::authorize_child_protection_access_for_section` (ADR-0072) **directly, unchanged** -- no new sibling function, no new `Capability` (see ADR-0083's explicit reuse-vs-fork rationale). `commands::anecdotal_record` (`record_anecdotal_entry`, `list_anecdotal_records_for_section`, `add_anecdotal_record_followup`, `list_anecdotal_record_followups`); create-only, no update/delete on narrative content. `GuidanceRecordsScreen` UI (section/roster pickers, record form, per-record follow-up log and add-follow-up form), reachable from "Learner Records" as "Guidance Records," axe-clean. Sync wiring: `EntityKind::AnecdotalRecord`/`AnecdotalRecordFollowup`, `upsert_record_from_sync`/`upsert_followup_from_sync`, sync-aware commands (enrollment-gated encrypt-on-enqueue, `authorize_child_protection_access_for_section` verified to survive unchanged through the sync path), `apply_decrypted_change` arms, `ConflictEntityPreview::AnecdotalRecord`/`AnecdotalRecordFollowup` typed variants. No natural-key-collision test -- confirmed no `UNIQUE` constraint beyond `id` on either table, matching `BehavioralIncident`/`IncidentIntervention`'s own precedent. **This batch does NOT wire `AnecdotalRecord` into `award-eligibility.ts`'s hardcoded-false disciplinary-anecdotes check** -- that remains a future batch's job; the entity is now ready for it. `cargo test`/`cargo clippy --all-targets -- -D warnings`/`cargo fmt --check` and `npm run quality` all pass.
- Schedule Grids: no persisted entity exists yet -- nothing to wire until the entity itself is built.
