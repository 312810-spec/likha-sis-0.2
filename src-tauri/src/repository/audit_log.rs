...
pub enum AuditEventType {
    ...
    /// `auth::admin_reset_teacher_password` (Wave 3N). The only event
...
pub struct AuditLogEntry {
    ...
    /// `PasswordResetByAdmin` (migration 24, Wave 3N). Every event
...
/// only `admin_reset_teacher_password` (Wave 3N): `actor_user_id` is the
