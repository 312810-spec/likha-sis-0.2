...
pub enum Capability {
    ...
    /// Generate school-wide official-form summaries (currently SF4 and
    /// SF6). These reports cross section boundaries and disclose school-
    /// wide attendance or promotion data, so a plain Teacher session is
    /// insufficient. Registrar and School Head are the current records-
    /// authority roles; a future LIS Coordinator role can widen this one
    /// mapping without changing command code.
    ExportSchoolReports,
}
...
impl Capability {
    fn allowed_roles(self) -> &'static [&'static str] {
        match self {
            ...
            Capability::ExportSchoolReports => &[role_repo::REGISTRAR, role_repo::SCHOOL_HEAD],
        }
    }
}
...
/// Admin-Assisted Password Reset (Wave 3N, ADR-0060): a School Head sets
...
mod tests {
    ...
    // ---- Wave 3N: admin_reset_teacher_password (ADR-0060) ----
