...
pub fn migrations() -> Migrations<'static> {
    Migrations::new(vec![
        ...
        M::up(
            r#"
        -- M41: widens the `entity_kind` allowlist to add
        -- `subject_attendance_entry` -- the per-learner attendance mark
        -- (`subject_attendance_entries`), as opposed to the
        -- session-level `subject_attendance` row already wired. Same
        -- 12-step CHECK-widening rebuild as migrations 24, 26, and 36
...
mod tests {
    ...
    #[test]
    fn migration_41_widens_entity_kind_to_accept_subject_attendance_entry_and_preserves_existing_rows(
    ) {
        let mut conn = Connection::open_in_memory().unwrap();
        conn.pragma_update(None, "foreign_keys", "ON").unwrap();
        migrations().to_latest(&mut conn).unwrap();
        ...
        for table in [
            ...
            let sql = match table {
                "sync_outbox" => "INSERT INTO sync_outbox
                    (change_id, school_id, device_id, actor_user_id, entity_kind, entity_id,
                     base_version, operation, encrypted_payload)
                 VALUES ('c2', 's1', 'd1', 'u1', 'subject_attendance_entry', 'e1', 0, 'upsert', X'01')",
                "sync_hub_log" => "INSERT INTO sync_hub_log
                    (change_id, school_id, device_id, actor_user_id, entity_kind, entity_id,
                     version, operation, encrypted_payload)
                 VALUES ('c3', 's1', 'd1', 'u1', 'subject_attendance_entry', 'e1', 1, 'upsert', X'01')",
                "sync_conflict_review" => "INSERT INTO sync_conflict_review
                    (id, change_id, school_id, device_id, actor_user_id, entity_kind, entity_id,
                     submitted_base_version, current_hub_version, operation, encrypted_payload)
                 VALUES ('r1', 'c4', 's1', 'd1', 'u1', 'subject_attendance_entry', 'e1', 0, 1, 'upsert', X'01')",
                "sync_version_cache" => "INSERT INTO sync_version_cache
                    (school_id, entity_kind, entity_id, known_version)
                 VALUES ('s1', 'subject_attendance_entry', 'e1', 1)",
                _ => unreachable!(),
            };
            conn.execute(sql, [])
                .unwrap_or_else(|e| panic!("{table} must accept 'subject_attendance_entry': {e}"));
        }
