...
fn export_sf5_as_current_session(
    ...
    let periods = grading::list_by_school_year(conn, &session_school_id, school_year)?;
    ...
    debug_assert_eq!(school_id, session_school_id);
...
#[test]
fn sf5_authorization_uses_the_schools_actual_last_grading_period_date() {
    ...
    let result = export_sf5_as_current_session(&conn, &sessions, &section.id, "2026-2027");
    assert!(result.is_ok());
}
...
fn export_sf6_as_current_session(
    ...
    let school_id =
        auth::authorize_capability(conn, sessions, auth::Capability::ExportSchoolReports)?;
...
#[test]
fn school_wide_sf4_and_sf6_exports_require_records_authority() {
    let conn = open_test_db();
    let school = school::create(&conn, "School Alpha").unwrap();
    let teacher = user::create_user(&conn, "teacher.alpha", "pw", "Teacher Alpha").unwrap();
    user::add_school_membership(&conn, &teacher.id, &school.id).unwrap();
    role::grant(&conn, &teacher.id, &school.id, role::TEACHER).unwrap();

    let sessions = SessionManager::new();
    auth::login(&conn, &sessions, "teacher.alpha", "pw", &school.id).unwrap();

    assert!(matches!(
        export_sf6_as_current_session(&conn, &sessions, "2026-2027"),
        Err(AppError::Unauthorized)
    ));
    assert!(matches!(
        export_sf4_as_current_session(&conn, &sessions, 2026, 9),
        Err(AppError::Unauthorized)
    ));

    role::grant(&conn, &teacher.id, &school.id, role::REGISTRAR).unwrap();
    assert!(export_sf6_as_current_session(&conn, &sessions, "2026-2027").is_ok());
    assert!(export_sf4_as_current_session(&conn, &sessions, 2026, 9).is_ok());
}
