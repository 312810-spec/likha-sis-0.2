...
export function SectionsScreen({
  ...
  const availableSchoolYears = Array.from(
    new Set(sections.map((section) => section.schoolYear).filter(Boolean)),
  );
  ...
  async function handleExportSf6(event: FormEvent) {
    ...
    try {
      const effectiveSchoolYear = sf6SchoolYear.trim() || availableSchoolYears[0] || "";
      if (!effectiveSchoolYear) {
        setSf6Error("Please enter or select a school year to export SF6.");
        return;
      }
      setSf6Result(await exportService.exportSchoolEosySf6(effectiveSchoolYear));
    } catch (err) {
      setSf6Error(
  ...
  return (
    <section aria-label="Sections">
      ...
      {exportService && (
        <section aria-label="School Form 6 (SF6) Summarized Promotion Report">
          <h3>End-of-School-Year Summary (SF6)</h3>
          {mode === "guided" && (
            <p className="field-hint">
              SF6 summarizes promotion decisions and proficiency levels across the school for the
              selected school year. Review the generated file before official submission.
            </p>
          )}
          ...
          {sf6Result && (
            <Alert tone="success">
              ...
              <p>
                This DepEd SF6 summary contains promotion decisions and proficiency totals for the
                selected school year.
              </p>
              {sf6Result.disclosure.omittedFields.length > 0 && (
                <>
                  <p>Complete these items manually:</p>
                  <ul>
              ...
              <div className="field">
                ...
                {availableSchoolYears.length > 0 ? (
                  <select
                    ...
                    onChange={(event) => setSf6SchoolYear(event.target.value)}
                    disabled={sf6Exporting}
                  >
                    {availableSchoolYears.map((schoolYearOption) => (
                      <option key={schoolYearOption} value={schoolYearOption}>
                        {schoolYearOption}
                      </option>
                    ))}
                  </select>
