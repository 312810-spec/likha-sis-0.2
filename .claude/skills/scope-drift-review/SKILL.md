...
3. **Configuration changes** — did `.claude/settings.json`,
...
3. If the same _kind_ of drift keeps recurring across sessions: that's
   ...
   (`.claude/rules/architecture.md`, `scripts/check-architecture.mjs`),
...
`.claude/rules/architecture.md` and `.claude/rules/security-privacy.md`.
